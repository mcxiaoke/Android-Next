...
public class HttpJobBuilder<T> {
    ...
    public List<ResponseProcessor<T>> mProcessors;
    ...
    public HttpJobBuilder<T> processor(ResponseProcessor<T> processor) {
        if (processor != null) {
            if (mProcessors == null) {
                mProcessors = new ArrayList<ResponseProcessor<T>>(4);
            }
